...
- R4. Split the install area into a standalone `Install` section and place it above `How to use` in the page flow.
- R5. Keep platform selectors as in-section controls (no links), with `Mac/Linux` active by default and `Windows` visible in unsupported state.
- R6. Platform switching inside install must not navigate or change hash/path; no scroll jump is allowed.
- R7. Keep the verify-caption area in a fixed visual region so switching platforms does not change vertical rhythm.

**Proof section**
- R8. Remove the entire proof section (the `Product proof` block, the video placeholder, and the duplicated screenshot cards). The two Android screenshots continue to appear as device frames inside the hero visuals, so proof is not lost. When the walkthrough video exists, a new proof section will be reintroduced with a different layout.

**Android / iOS distribution**
- R9. Remove the Android APK and iOS CTA buttons from the install strip. No public APK URL exists, and there is no iOS build.
- R10. Add a compact "Get the Android app" card after the how-it-works section explaining that the APK is invite-only, with two contact affordances: a `mailto:yuanbo.han@gmail.com` link and a link to `https://github.com/yuanbohan`. One short sentence of framing is enough; no form, no newsletter.

**Geek-native tone**
- R11. Keep the existing dark grid + Chivo + IBM Plex Mono aesthetic. Do not introduce brand-site tropes (gradients, marquees, trust logos, stats counters). Lean the copy further into plain-spoken developer voice; avoid marketing superlatives.
- R12. Keep the `Private preview` pill in the hero — it accurately signals the invite-only stage and fits the geek voice.
- R13. Update the footer note to match the current reality: it should no longer reference swapping in an install URL or Android APK, since those are resolved or deferred. A one-line footer that names the project and the early-stage status is enough.

**How to use section**
- R14. Keep `How to use` as a distinct section below `Install`, using the existing 4-step flow.
- R15. Do not place install platform controls inside `How to use`.
...
- Platform switching in `Install` does not trigger navigation or scroll-jump in interaction.
...
- **Drop the whole proof section instead of reshuffling.** The video placeholder was the main reason for the two-column layout; with it gone, the screenshots are already carried by the hero visuals. Keeping a half-empty proof section would read as unfinished. When the video arrives, a fresh layout decision is cheaper than maintaining a transitional one.
...
- **Fix page sequence to `Install` then `How to use`.** This keeps the onboarding path linear and avoids misreading install options as usage steps.
- **Make install platform switching in-panel only.** Platform selection should switch content visibility, not navigation.
...
- [Affects R8][Technical] Whether to delete the proof-related fields from `src/site-content.js` and the matching blocks in `src/main.js` and `src/styles.css`, or keep the CSS/content scaffolding commented out for the later video return. Recommend deleting now and reintroducing fresh when the video exists.
- [Affects R10][Technical] Exact placement and markup of the contact card — a new section between `how-it-works` and the footer vs. an expanded footer. Planning can pick based on existing section rhythm.
